...
/*
 * Import Driver for type: association
 */
...
class ImportDriver_association extends ImportDriver_default
{
    ...
    {
        // Import association field:
        $related_field_ids = $this->getRelatedField();
        $data = explode(',', $value);
        ...
        foreach ($related_field_ids as $related_field_id) {
         foreach ($data['relation_id'] as $relation_id)
         {
             if(!empty($relation_id))
             {
                 ...
                 if (isset($row['value'])) {
                     $related_values[] = '(' . $relation_id . ') ' . trim($row['value']);
                 } else {
    ...
    public function scanDatabase($value)
    {
        ...
        // If there is a matching result, it means the entry exists in the  section
